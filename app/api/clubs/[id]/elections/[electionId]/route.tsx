...
export async function PUT(
  ...
  try {
    ...
    const now = new Date();
    const isActive =
      now >= existingElection.votingStart && now <= existingElection.votingEnd;
    ...
    const updatedElection = await prisma.$transaction(async (tx) => {
      ...
      const election = await tx.election.update({
        ...
        data: {
          ...
          subtitle: body.subtitle ?? existingElection.subtitle,
          description: body.description ?? existingElection.description,
    ...
    return NextResponse.json(completeElection);
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    // Check if voting has started (prevent deletion of active elections)
    const now = new Date();
