...
const EditElectionModal: React.FC<EditElectionModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    votingStart: "",
    votingEnd: "",
    positions: [] as Election["positions"],
  });
  ...
  useEffect(() => {
    if (election && isOpen) {
      setFormData({
        ...
        year: election.year,
        votingStart: new Date(election.votingStart).toISOString().slice(0, 16),
        votingEnd: new Date(election.votingEnd).toISOString().slice(0, 16),
  ...
  const updateCandidate = (
    ...
    const updatedPositions = [...formData.positions];
    (
      updatedPositions[positionIndex].candidates[candidateIndex] as {
  ...
  const hasStarted = new Date() >= new Date(election.votingStart);
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[95vh] overflow-hidden border-2 border-gray-100 transform transition-all duration-300 animate-scale-in">
          ...
          <div className="relative flex justify-between items-center p-5">
            <div className="space-y-1">
              ...
              <p className="text-gray-700 font-medium ml-10 text-sm">
                Update your democratic process with precision
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        >
          <div className="p-5 space-y-6">
            ...
            {hasStarted && (
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-4 border-l-4 border-yellow-500 shadow-lg animate-slide-in">
                <p className="text-yellow-800 font-semibold text-sm">
                  Note: This election has already started. Some changes may be
                  restricted.
                </p>
              </div>
            )}
